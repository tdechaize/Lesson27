#ifndef HEADER_CC990EDCC43F09C4
#define HEADER_CC990EDCC43F09C4
...
struct sPlane{
	unsigned int p[3];
	struct sPoint normals[3];
	unsigned int neigh[3];
	struct sPlaneEq PlaneEq;
	bool visible;
};
...
struct glObject{
	...
	struct sPoint points[100];
	struct sPlane planes[200];
};

// load object
int ReadObject(char *st, struct glObject *o){
  FILE *file;
  unsigned int i;
...
void SetConnectivity(struct glObject *o){
...
void CalcPlane(struct glObject *o, struct sPlane *plane){
	struct sPoint v[4];
	int i;
...
void DrawGLObject(struct glObject *o){
	unsigned int i, j;
...
void  CastShadow(struct glObject *o, float *lp){
